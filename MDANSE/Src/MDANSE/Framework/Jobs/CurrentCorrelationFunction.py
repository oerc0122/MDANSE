...
class CurrentCorrelationFunction(IJob):
    ...
    def run_step(
        self, index: int
    ) -> tuple[int, tuple[npt.NDArray[complex], npt.NDArray[complex]] | None]:
        ...
        if (
            ...
            or not self.generator.q_vectors[shell]["q_vectors"].size
        ):
            return index, None
