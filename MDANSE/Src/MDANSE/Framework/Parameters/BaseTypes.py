...
from MDANSE.Framework.Parameters.UtilTypes import CB, Depends, K, Num, V
...
class NumericRange(Generic[Num]):
    ...
    def __init__(self, *params: Num):
        try:
            self.iterator: numeric_range[Num, Num] = numeric_range(*params)
...
class Boolean(ConfigureDescriptor[Bools, bool, CB]):
    """
...
class Float(MinMax[float], ConfigureDescriptor[SupportsFloat, float | None, CB]):
...
class Integer(MinMax[int], ConfigureDescriptor[SupportsInt, int | None, CB]):
    """Integer parameter."""
...
class Dict(
    ...
    def __init__(self, *, fixed: bool = False, choices: Sequence[K] = (), **params):
        super().__init__(choices=choices, **params)  # type: ignore

        self.fixed = fixed

    def _validate_choices(
        self,
        value: dict[K, V],  # type: ignore[override]
        choices: set[K] | None = None,  # type: ignore[override]
    ) -> bool:
    ...
    def validate(
        self,
        value: Iterable[tuple[K, V]] | dict[K, V] | str | Path | None,
        deps: Depends,
        /,
    ) -> dict[K, V] | None:
        if self.optional and value is None:
        ...
        value = cast("dict", super().validate(value, deps))
...
class String(ConfigureDescriptor[str, str, CB]):
    """String parameter.
...
class PathParam(ConfigureDescriptor[str | Path, Path, CB]):
    """Path parameter.
    ...
    def __init__(
        ...
        *,
        ...
        on_set: Callable[[Self, Path, Depends], CB] | None = None,
        **params,
    ):
...
class Array(ConfigureDescriptor[str | npt.ArrayLike, np.ndarray, CB]):
    """Array parameter.
