...
def test_lammps_mdt_conversion_file_exists_and_loads_up_successfully(compression):
    ...
    with h5py.File(temp_name + ".mdt") as actual, h5py.File(result_file) as desired:
        ...
        np.testing.assert_array_almost_equal(
            actual["/unit_cell"], desired["/unit_cell"]
        )
...
def test_discover_mdt_conversion_file_exists_and_loads_up_successfully(compression):
    ...
    with h5py.File(temp_name + ".mdt") as actual, h5py.File(result_file) as desired:
        ...
        np.testing.assert_array_almost_equal(
            actual["/configuration/velocities"], desired["/configuration/velocities"]
        )
        np.testing.assert_array_almost_equal(
            actual["/unit_cell"], desired["/unit_cell"]
        )
...
def test_cp2k_mdt_conversion_file_exists_and_loads_up_successfully(velocity):
    ...
    with h5py.File(temp_name + ".mdt") as actual, h5py.File(result_file) as desired:
        ...
        if velocity:
            np.testing.assert_array_almost_equal(
                actual["/configuration/velocities"],
                desired["/configuration/velocities"],
            )
        np.testing.assert_array_almost_equal(
            actual["/unit_cell"], desired["/unit_cell"]
        )
...
def test_dlp_mdt_conversion_file_exists_and_loads_up_successfully_with_dlp_with_grad(
    ...
    with h5py.File(temp_name + ".mdt") as actual, h5py.File(result_file) as desired:
        ...
        np.testing.assert_array_almost_equal(
            actual["/configuration/gradients"], desired["/configuration/gradients"]
        )
        np.testing.assert_array_almost_equal(
            actual["/unit_cell"], desired["/unit_cell"]
        )
